...
export const HappinessConfig = {
     ...
     */
    fiscalSponsorMode: (process.env.NEXT_PUBLIC_FISCAL_SPONSOR_MODE as string || 'true').toLowerCase() === 'true',
    /**
     ...
     */
    hidePoweredByHappiness: (process.env.NEXT_PUBLIC_HIDE_POWERED_BY as string || 'false').toLowerCase() === 'true',
    /**
     * Tell Stripe to issue an invoice after checkout. This generally costs 0.4% of the transaction amount up to $2, so we default to false.
     */
    enableInvoiceAfterCheckout: (process.env.NEXT_PUBLIC_ENABLE_INVOICES as string || 'false').toLowerCase() === 'true',
} as const;
