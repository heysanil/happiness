...
import { donors, selectDonationSchema, selectDonorSchema } from '@db/schema';
...
export const getDonor = async (
    ...
    if (
        ...
        throw new HappinessError('Missing or invalid donor ID or email', 400, {
            search,
        });
    }

    const query = await db.query.donors.findFirst({
        where: validate(search)
            ? eq(donors.email, search)
            : eq(donors.id, search),
        with: {
    ...
    if (!query) {
        throw new HappinessError('No such donor found', 404, {
            queryReturn: query,
        });
    }
    ...
    return validateReturn(
        options?.include?.donations
            ? selectDonorSchema.merge(
                  z.object({ donations: z.array(selectDonationSchema) }),
              )
            : selectDonorSchema,
        query,
    );
