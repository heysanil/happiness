...
const PagesColumns = {
    id: varchar('id', { length: 20 })
        .primaryKey()
        .$defaultFn(() => generateID(Prefixes.Page)),
...
export const DonationsColumns = {
    id: varchar('id', { length: 20 })
        .primaryKey()
        .$defaultFn(() => generateID(Prefixes.Donation)),
    createdAt: timestamp('created_at', { fsp: 3 })
        .notNull()
        .default(sql`CURRENT_TIMESTAMP(3)`),
    updatedAt: timestamp('updated_at', { fsp: 3 })
    ...
    amountCurrency: mysqlEnum('amount_currency', ['usd'])
        .notNull()
        .default('usd'),
    ...
    externalTransactionProvider: mysqlEnum('external_transaction_provider', [
        'stripe',
    ]),
...
export const DonorsColumns = {
    id: varchar('id', { length: 20 })
        .primaryKey()
        .$defaultFn(() => generateID(Prefixes.Donor)),
    createdAt: timestamp('created_at', { fsp: 3 })
        .notNull()
        .default(sql`CURRENT_TIMESTAMP(3)`),
    updatedAt: timestamp('updated_at', { fsp: 3 })
